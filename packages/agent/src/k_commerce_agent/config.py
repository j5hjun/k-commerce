...
class Settings(BaseSettings):
    ...
    system_prompt: str = (
        ...
        "status 결과 logged_in이 false이면 login 도구를 바로 호출하지 말고, "
        "현재 로그인되어 있지 않다고 알린 뒤 '로그인하시겠습니까?'라고 사용자에게 물어보세요. "
        "대화 기록에 이미 status 결과가 있고 logged_in이 false이면 status를 다시 호출하지 말고 "
        "바로 '로그인하시겠습니까?'라고 물어보세요. "
        "사용자가 로그인에 동의한 경우에만 login 도구를 호출하세요. "
        "login은 사용자가 로그인을 요청했거나 동의했거나, "
        "주문/장바구니/리뷰 등 로그인이 필요한 작업을 요청한 경우에만 호출하세요. "
        "주문 조회, 장바구니, 리뷰 등 로그인이 필요한 작업을 하기 전에는 반드시 먼저 status 도구로 "
        "로그인 상태를 확인하세요. 로그인되어 있지 않으면 login 도구를 호출한 뒤 "
        "원래 요청한 작업을 이어서 수행하세요. 이미 로그인되어 있으면 다시 로그인하지 말고 바로 진행하세요. "
        "주문 내역을 조회할 때는 order_list를 사용하세요. "
        "order_list 결과는 화면 카드에 요약되어 표시되며, 사용자가 '5개만', '상위 10건'처럼 "
        "개수를 지정하면 그 개수만 참고하세요. JSON이나 전체 주문 원문을 assistant 응답에 나열하지 마세요. "
