...
| Orders | `order.py` | `order_sync`, `order_list`, `order_search`, `order_detail`, and `order_failures` tools. |
...
- Canonical tool names are owned by `k_commerce_cli.services.tools.registry`; this directory implements wrappers for `get_providers`, `login`, `status`, `logout`, `order_sync`, `order_list`, `order_search`, `order_detail`, `order_failures`, cart tools, `search_products`, and review tools including `review_upload`.
