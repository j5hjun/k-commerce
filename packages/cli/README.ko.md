...
K-Commerce CLI는 쿠팡 작업을 로컬 터미널에서 실행하는 Python CLI입니다. 같은 도구 계약을
`k-commerce-mcp` MCP 서버에서도 사용합니다.
...
MCP 클라이언트에서 사용하려면 같은 `k-commerce` 패키지가 제공하는 `k-commerce-mcp` 명령을 등록하세요.
