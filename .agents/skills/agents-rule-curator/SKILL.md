...
1. Identify the target `AGENTS.md`.
   - Prefer the nearest scoped file for the rule, such as `packages/cli/tests/smoke/AGENTS.md` for smoke-test rules.
...
- Prefer direct instructions: "Place order smoke tests under `smoke/order/`" instead of "We decided to place..."
...
For a conversation where the user selects rules 2, 3, and 4 from a smoke-test discussion, add only those selected rules to the scoped smoke-test `AGENTS.md`. Do not add unselected candidates, and do not rewrite unrelated file-map entries unless the selected rule requires it.
