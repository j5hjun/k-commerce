...
5. Edit maps.
   ...
   - Use short headings such as `# Repository Map`, `# Package File Map`, or `# Tests File Map`.
...
- In source directories, describe ownership and runtime role: entry points, service abstractions, adapters, storage, and type definitions.
...
This directory contains the service layer.

- `base.py` - shared service protocols.
- `registry.py` - lookup and list helpers.
- `store.py` - persistent artifact storage.
- `adapters/` - concrete external-system adapters. See `adapters/AGENTS.md`.
```
