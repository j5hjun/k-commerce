...
def test_failed_build_never_changes_version(tmp_path, monkeypatch):
    version_file = tmp_path / "VERSION"
    version_file.write_text("0.1.3\n")
    monkeypatch.setattr(build_release, "__file__", str(tmp_path / "scripts/ci/build_release.py"))
    monkeypatch.setattr("sys.argv", ["build_release.py", "--output", str(tmp_path / "release")])

    def fail_build(command, **kwargs):
        assert version_file.read_text() == "0.1.3\n"
